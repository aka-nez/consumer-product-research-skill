...
> **Superseded implementation record:** The initial no-MCP architecture, tool-neutral skill, and grader bodies embedded below are historical. The current account-authenticated Firecrawl availability-proof contract in `docs/superpowers/specs/minimal-claude-cowork-harness.md`, `.mcp.json`, `skills/consumer-product-research/SKILL.md`, and `evals/` is authoritative.
